...
export default function AdminPanel() {
  ...
  useEffect(() => {
    ...
    axios
      .get("http://51.20.73.229/api/admin-category/all")
  ...
  //   await axios.post('http://51.20.73.229/api/admin-category/category', { name: categoryName }, headers);
  ...
  const handleAddCategory = async () => {
    ...
    try {
      await axios.post(
        "http://51.20.73.229/api/admin-category/category",
        { name: categoryName },
        headers
      );
      alert("Category added");
      ...
      const res = await axios.get(
        "http://51.20.73.229/api/admin-category/all",
        headers
      );
  ...
  const handleUploadFile = async () => {
    ...
    await axios.post(
      `http://51.20.73.229/api/admin-category/category/${categoryId}/upload`,
      formData,
      headers
    );
  ...
  //   await axios.put(`http://51.20.73.229/api/admin/question/${questionId}/options`, {
  //     options,
  //     correctAnswer
  //   }, headers);
  //   alert('Options saved');
  ...
  const handleUpdateOptions = async (questionId) => {
    ...
    try {
      await axios.put(
        `http://51.20.73.229/api/admin/question/${questionId}/options`,
  ...
  const handleUpdateSettings = async () => {
    await axios.put(
      `http://51.20.73.229/api/admin/test-settings`,
      settings,
      headers
    );
  ...
  const fetchQuestions = async () => {
    ...
    try {
      const res = await axios.get(
        `http://51.20.73.229/api/admin-category/category/${categoryId}/questions`,
        headers
      );
    ...
    <div className="admin-container">
      ...
      <div className="admin-panel-box">
        <div className="admin-tabs">
          {["category", "upload", "options", "settings", "view", "logout"].map(
            (key) => (
              <button
                key={key}
                onClick={() => setTab(key)}
          ...
          {tab === "category" && (
            <>
              ...
              <button
                ...
                className="admin-button btn-blue"
              >
                Add Category
              </button>
            </>
          ...
          {tab === "upload" && (
            <>
              ...
              <button
                ...
                className="admin-button btn-green"
              >
                Upload
              </button>
            </>
          ...
          {tab === "options" && (
            <>
              ...
              <button
                ...
                className="admin-button btn-purple"
              >
                Save Options
              </button>
            </>
          ...
          {tab === "settings" && (
            <>
              ...
              <button
                ...
                className="admin-button btn-yellow"
              >
                Update Settings
              </button>
            </>
...
function QuestionEditor({ question, index, headers, onSave }) {
  ...
  const [localCorrect, setLocalCorrect] = useState(
  ...
  const saveOptions = async () => {
    ...
    try {
      await axios.put(
        `http://51.20.73.229/api/admin/question/${question._id}/options`,
  ...
  return (
    <div className="question-boxes">
      ...
      {question.options ? (
        ...
        <p style={{ color: "gray" }}>
          <em>No options provided</em>
        </p>
      )}
      <p>
        <strong>Correct Answer:</strong> {question.correctAnswer || "Not Set"}
      </p>
        ...
        {["A", "B", "C", "D"].map((opt) => (
          <div key={opt} className="option-input-group">
            ...
            <input
              ...
              onChange={(e) =>
                setLocalOptions({ ...localOptions, [opt]: e.target.value })
              }
              className="admin-input"
            />
          </div>
